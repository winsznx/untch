...
**Version:** 2.3 (control-channel authority boundary + judge-proofed Photon claims) · **Status:** Approved direction, pre-build · **Owner:** Tim (@winsznx) · **Target:** OKX AI Genesis Hackathon → standalone company
...
- **S1 — Untch MCP Server** (A2MCP listing #1): pay-per-call tools (§11), OKX Payment SDK integrated. Primary product + eligibility artifact.
...
```json
{
  ...
  "perCallCap": 1.0, "escalateAbove": 5.0, "escalationTimeoutMin": 30,
...
Error envelope `{code, message, retryable, docsUrl}` · free tier: first 20 preflights per new agent · every paid call settles via OKX Payment SDK — our revenue is itself visible marketplace revenue.
...
0. **Execute the Day 0 gate (§29)** — D0.1 Payment SDK hello-world through D0.9; hard gate on D0.1/D0.3/D0.4/D0.5 before any feature work.
...
3. **OKX Payment SDK specifics** — *resolved at D0.1*: integration contract, settlement token(s), payout cadence, metadata fields; §11 mechanics adjust to what the SDK actually supports.
...
| Payment SDK diverges from assumptions | Build task #0 is the paid hello-world before feature work |
...
|---|---|---|
| **D0.1** | **OKX Payment SDK hello-world** — one *paid* A2MCP call end-to-end (register minimal tool, price it, call it, observe settlement). Resolves §23 Q3 | request/response log + settlement record + integration notes committed to /internal |
...
| **D0.3** | **X Layer basics** — chain IDs, mainnet + testnet RPCs, explorer, faucet; **USDT/USDG contract addresses + decimals from the official token list** (resolves Q5); ops wallet funded with OKB gas | `chains.ts` constants file + funded-wallet tx link |
