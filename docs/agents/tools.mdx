...
| POST | `/create_spend_intent` | bundled | Validate, hash and bind the intent, with optional on-chain register |
...
| POST | `/redact_payment_metadata` | $0.02 | Strip PII patterns and return a hash |
...
`brand_pack` is the one-call hire path. Domain checks use live RDAP (not a purchase or reservation). LLM is optional for naming only, producing Untch-style short brands with AI-slop stems banned. Policy and preflight stay LLM-free.
