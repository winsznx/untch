...
`(intent, policy, ledgerWindow)`, and there is no network client in the package at all.
...
The x402 v2 challenge arrives in the `payment-required` **response header** as base64 JSON, and the body
...
ceiling and one recipient allowlist, redeemed once under a row lock **before** signing.
...
remaining 0.050000 sits in `CROSS_RAIL_CLEARING` on X Layer, which is value the funding rail owes the
settlement rail. It retires when an operator sweeps it.
...
3. **Receipt anchoring is on X Layer testnet.** The mainnet `UntchReceipts` writer is not yet
   ...
   durable and report `ANCHOR_FAILED` honestly. **The ledger is authoritative regardless, because anchoring
