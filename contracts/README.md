...
|------|-----------|--------|
| [`src/PolicyRegistry.sol`](src/PolicyRegistry.sol) | PRD §10.1 — on-chain anchor: a committed ruleset (`policyHash`) governed a given agent at a given time. Owner-gated register / update / pause / resume, event per mutation. | **Real.** Full §28 pipeline green; deployed + proven on local anvil; **X Layer testnet deploy pending gas** (see below). |
...
| **Fork / testnet deploy** | ⛔ live X Layer testnet deploy blocked on faucet gas — see [`deploy/README.md`](deploy/README.md). Path proven end-to-end on local anvil ([`deploy/anvil-proof-receipt.json`](deploy/anvil-proof-receipt.json)). |
