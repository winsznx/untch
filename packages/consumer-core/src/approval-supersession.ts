...
export async function supersedePriorQuote(
  ...
  await tx.query(
    ...
    [args.priorApprovalRequestId, args.reason, args.quoteLineageId],
  );
  ...
  /** Bidirectional, so a timeline can be walked from either end. */
  await tx.query(
    `UPDATE untch_approval_requests
        SET superseded_by_approval_request_id = $2
      WHERE approval_request_id = $1`,
    [args.priorApprovalRequestId, args.newApprovalRequestId],
  );
  await tx.query(
    `UPDATE untch_approval_requests
