/**
 * The Agentic Wallet link, with no transport in it.
 *
 * WHY THIS IS THE PRIMARY PATH AND THE BROWSER ONE IS NOT
 *
 * `/consumer/account/link/*` assumes one browser: a request starts, and the same browser returns with
 * a signature moments later. That is how an injected EIP-1193 provider works, and it is not how the
 * Agentic Wallet works at all. The Agentic Wallet is held in OKX's TEE, restored through email,
 * Google or Apple login, and reached through the `onchainos` CLI or skill. Nothing is injected into a
 * page, and there is no provider to call — a flow built on `window.ethereum` reaches the OKX browser
 * EXTENSION, which is a different wallet product with different keys.
 *
 * The Cloudflare port shipped the browser routes and left these four refusing, which inverted the
 * design: the secondary path was the only one served, and the wallet Untch is actually for could not
 * be linked at all.
 *
 * WHAT IS UNCHANGED, DELIBERATELY
 *
 * All of the cryptography. Server-authored message, server-minted single-use nonce, expiry, domain
 * binding, chain binding, scope Resources, signature recovery, replay refusal and the
 * operational-address collision check are the ones Express runs. Only the transport differs: a
 * browser starts the request and WAITS, an agent fetches the challenge and signs inside the TEE, and
 * the browser learns it worked by polling.
 *
 * WHAT IS NEVER STORED
 *
 * No email, OTP, login session, API secret, mnemonic or key. Email authenticates access to the wallet
 * AT OKX; it is not spending authority here and there is nowhere in this schema to put it.
 */
...
export async function handleAgenticStart(
  body: unknown,
  deps: AgenticLinkDeps,
  ...
  const now = deps.now;
    ...
    const { request } = await deps.links.create({
      requestedScopes: scopes,
      context: {
    ...
    const linkUrl = `${deps.webBaseUrl}/link/${request.linkRequestId}`;
    ...
    return {
      ...
      body: {
        ...
        pollUrl: `${deps.publicBaseUrl}/consumer/account/agentic-link/${request.linkRequestId}/status`,
        challengeUrl: `${deps.publicBaseUrl}/consumer/account/agentic-link/${request.linkRequestId}/challenge`,
        purpose:
        ...
        agentPrompt:
          ...
          `${deps.publicBaseUrl}. Fetch the challenge, show me the exact message and the address it will be ` +
...
export async function handleAgenticChallenge(
  linkRequestId: string,
  /**
   * The address the agent is about to sign with, threaded in rather than read off a request.
   *
   * It is a QUERY parameter on Express because this GET may legitimately repeat: a user who switches
   * sub-wallet re-fetches with a different address and gets a message naming the new one. The nonce
   * does not change, so it is still one request.
   */
  address: string | null,
  deps: AgenticLinkDeps,
): Promise<HandlerResult> {
  const now = deps.now;
    ...
    if (raw !== null) await deps.links.markChallengeFetched({ linkRequestId, expectedAddress: raw, nowMs: now() });
...
export async function handleAgenticComplete(
  linkRequestId: string,
  body: unknown,
  deps: AgenticLinkDeps,
): Promise<HandlerResult> {
  const now = deps.now;
    ...
    const proof = await verifyWalletProof(
      { message, signature: signature as Hex, expectedNonce: request.siweNonce, domain: deps.domain, nowMs: now() },
      deps.verifier,
    );
    ...
    const account = existing ?? (await deps.accounts.createAccount({ by: "agentic-wallet" }));
     ...
     */
    const redeemed = await deps.links.redeemVerified({
      linkRequestId,
    ...
    await deps.accounts.setPrimaryWallet({ accountId: account.accountId, bindingId, by: "agentic-wallet" });
    await deps.accounts.recordAuthentication({ accountId: account.accountId, by: "agentic-wallet" });
...
export async function handleAgenticStatus(
  linkRequestId: string,
  deps: AgenticLinkDeps,
): Promise<HandlerResult> {
  const now = deps.now;
